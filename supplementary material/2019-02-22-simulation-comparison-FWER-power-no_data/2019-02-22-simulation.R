...
main <- function() {
  ...
  default <- list(tau = 0.5, B = 100, n = 100, m = 6, pi0 = 0.5, effect = 0.2) #default setting

  compare_generator(tau, B, n, m, pi0, effect, default, seed)
}
...
compare_generator <- function(tau, B, n, m, pi0, effect, default, seed) {
...
perform_simulation <- function(var, default, seed, results = list()) {
  ...
  for (var_ in var) {
    ...
    results__ <- perform_test(default$tau, default$B, default$n, default$m, default$pi0, default$effect)
  ...
  save(results, file = "2019-02-22-simulation.RData")
  results
}

perform_test <- function(tau, B, n, m, pi0, effect) {
  ...
  results <- foreach(l = 1:L, .export = lsf.str(.GlobalEnv), .packages = "copula", .combine = rbind) %dorng% {
    ...
    for (try_ in 1:n_try) {
      try(silent = TRUE, {
        ...
        T <- qnorm(rCopula(1, C), sd = 1/sqrt(n))
        T[, (m0+1):m] <- T[, (m0+1):m] + effect
        C_star <- gumbelCopula(iTau(gumbelCopula(), tau), dim = m, use.indepC = "FALSE")
        T_star <- qnorm(rCopula(B, C_star), sd = 1/sqrt(n)) # sample of T under LFC
        ...
        p_values <- 2*(1 - pnorm(T, sd = 1/sqrt(n))) #two-sided #artificial distribution; no data is used
        test <- matrix(nrow = m, ncol = length(result)/2)
        test[, 1] <- p_values < alpha_loc.GNZ
        test[, 2] <- p_values < alpha_loc.MC
        test[, 3] <- p_values < alpha_loc.phi
        test[, 4] <- p_values < alpha/m
...
plot_simulation <- function(data, varname, title) {
  ...
  gg <- ggplot(data, aes(data[, varname], data[, short_title], color = type)) +
    ...
    ggtitle(title) +
    geom_line()
  ggsave(paste("2019-02-22-simulation-", varname, "-", short_title,
